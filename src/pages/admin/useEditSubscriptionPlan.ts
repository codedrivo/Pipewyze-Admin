  ...
  useEffect(() => {
    const fetchPlanDetails = async () => {
      ...
      try {
        ...
        if (res && res.plans) {
          const targetPlan = res.plans.find(
            (p: any) => (p._id || p.id) === id
          );
          if (targetPlan) {
            formik.setValues({
              name: targetPlan.name || "",
              tier: targetPlan.tier || "standard",
              ...
              featuresText: (targetPlan.features || []).join("\n"),
              stripeMonthlyPriceId: targetPlan.stripeMonthlyPriceId || "",
              stripeYearlyPriceId: targetPlan.stripeYearlyPriceId || "",
              isActive: targetPlan.isActive !== undefined ? targetPlan.isActive : true,
              order: targetPlan.order || 0,
            });

            if (targetPlan.accessibleFeatures && targetPlan.accessibleFeatures.length > 0) {
              setAccessibleFeatures(targetPlan.accessibleFeatures);
            } else {
              setAccessibleFeatures(
                SYSTEM_FEATURE_CATALOG.map((f) => ({ ...f, enabled: false }))
              );
            }
          }
        }
